...
def create_obstacles(s, e):
    ...
    for i in range(10):
        ...
        if location not in locations:
            ...
            obstacle = char.ObstacleClass(img_path, location, attribute)
            # 把实例对象添加到精灵组里面了
...
def Show_Start_Interface(Demo, width, height):
    ...
    tfont = pygame.font.Font(glv.get_var("font_set"), width//4)
    cfont = pygame.font.Font(glv.get_var("font_set"), width//20)
...
def over(Demo, width, height):
    ...
    overt= pygame.font.Font(glv.get_var("font_set"), width//4)
